...
pub struct Population<T> where T : Phenotype<T> + Ord + Clone {
    population: Vec<T>,
...
impl <T> Population<T> where T : Phenotype<T> + Ord + Clone {
    ...
    pub fn populate(&mut self) {
        for i in 0..self.population_cap {
    ...
    pub fn mutate(&mut self) {
        ...
        for (i, elem) in self.population.iter_mut().enumerate() {
            ...
            if chance > i {
                ...
                for (j, parent_two) in self.population.iter().enumerate() {
                    ...
                    if second_chance > j || j == (self.population.len() - 1) {
                        let mut children = parent_one.crossover( parent_two );
